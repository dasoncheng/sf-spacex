...
export class ActivationsController {
  ...
  @Post()
  async create(@Body() createActivationDto: CreateActivationDto): Promise<ActivationResponseDto> {
    return this.activationsService.create(createActivationDto);
  }

  @Post('check')
  async checkValidity(@Body() checkDto: ValidityCheckDto): Promise<ValidityResponseDto> {
